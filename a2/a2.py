...
def filter_by_first_name(records, first_name):
    ...
    return [i['id'] for i in records if i['first_name'].lower() == first_name]
...
def filter_by_last_name(records, last_name):
    ...
    return [i['id'] for i in records if i['last_name'].lower() == last_name]
...
def filter_by_full_name(records, full_name):
    ...
    return [i['id'] for i in records if (i['first_name'] + ' ' + i['last_name']).lower() == full_name]
...
def filter_by_age_range(records, min_age, max_age):
    ...
    - A list of INTEGERS denoting the IDs of the persons with the given full name
        ...
        Case 2: At least one person found => Returns a list containing the IDs of all the persons found
    """
    return [i['id'] for i in records if min_age <= i['age'] <= max_age]
...
def filter_by_address(records, address):
    ...
    a = []
    if address == {}:
        return [{'first_name': i['first_name'], 'last_name': i['last_name']} for i in records]
    for i in address:
        ...
        for i in records:
            ...
            for j in temp:
                if type(temp[j]) == str:
                    temp[j] = temp[j].lower()
            if temp == address:
                a.append({'first_name': i['first_name'], 'last_name': i['last_name']})
        return a
    for i in records:
        temp = i['address']
        for j in temp:
            if type(temp[j]) == str:
                temp[j] = temp[j].lower()
        if all(x in temp.items() for x in address.items()):
            a.append({'first_name': i['first_name'], 'last_name': i['last_name']})
    return a
...
def find_alumni(records, institute_name):
    ...
    a = []
    for i in records:
        for j in i['education'][::-1]:
            if j['institute'].upper() == institute_name and not j['ongoing']:
                a.append({'first_name': i['first_name'], 'last_name': i['last_name'], 'percentage': j['percentage']})
    return a


def topper_id(records, institute_name):
    """
...
def find_topper_of_each_institute(records):
    ...
    for person in records:
        for edu in person['education']:
            if edu['institute'].upper() not in a:
                a[edu['institute'].upper()] = topper_id(records, edu['institute'])
    return a
...
def find_blood_donors(records, receiver_person_id):
    ...
    """
    rin = search(records, receiver_person_id)
    if rin == -1:
        return {}
    b = records[rin]['blood_group']
    a = {}
    if b == 'AB':
        r = {i['id']: i['contacts'] for i in records}
        r.pop(receiver_person_id)
        return r
    elif b == 'A':
        r = {i['id']: i['contacts'] for i in records if i['blood_group'] == 'A' or i['blood_group'] == 'O'}
        r.pop(receiver_person_id)
        return r
    elif b == 'B':
        r = {i['id']: i['contacts'] for i in records if i['blood_group'] == 'B' or i['blood_group'] == 'O'}
        r.pop(receiver_person_id)
        return r
    elif b == 'O':
        r = {i['id']: i['contacts'] for i in records if i['blood_group'] == 'O'}
        r.pop(receiver_person_id)
        return r
...
def get_common_friends(records, list_of_ids):
    ...
    """
    ain = search(records, list_of_ids[0])
    if ain == -1:
        return []
    a = set(records[ain]['friend_ids'])
    for i in list_of_ids:
        x = search(records, i)
        if x == -1:
            return []
        b = set(records[search(records, i)]['friend_ids'])
        a = a & b
    return list(a)
...
def is_related(records, person_id_1, person_id_2):
    ...
    **** BONUS QUESTION ****
    ...
    """
    dic = {i['id']: False for i in records}
    dic[person_id_1] = True
    done = []
    while True:
        count = 0
        for i in dic:
            if dic[i]:
                ...
                for j in records[search(records, i)]['friend_ids']:
                    dic[j] = True
...
def delete_by_id(records, person_id):
    ...
    """
    p_index = search(records, person_id)
    if p_index == -1:
        return records
    f = records[p_index]['friend_ids']
    for i in f:
        records[search(records, i)]['friend_ids'].remove(person_id)
    records.pop(p_index)
    return records
...
def add_friend(records, person_id, friend_id):
    ...
    In case there were no updates, return the original records.
    """
    p, f = search(records, person_id), search(records, friend_id)
    if p == -1 or f == -1 or p == f:
        return records
    if friend_id not in records[p]['friend_ids']:
        records[p]['friend_ids'].append(friend_id)
        records[f]['friend_ids'].append(person_id)
    return records
...
def remove_friend(records, person_id, friend_id):
    ...
    if p == -1 or f == -1 or p == f:
        return records
    if friend_id in records[p]['friend_ids']:
        records[p]['friend_ids'].remove(friend_id)
        records[f]['friend_ids'].remove(person_id)
    return records
...
def add_education(records, person_id, institute_name, ongoing, percentage):
    ...
    In case there were no updates, return the original records.
    """
    p = search(records, person_id)
    if p == -1:
        return records
    if ongoing:
        records[p]['education'].append({'institute': institute_name, 'ongoing': ongoing})
        return records
    records[p]['education'].append({'institute': institute_name, 'ongoing': ongoing, 'percentage': percentage})
    return records
records=read_data_from_file()
find_topper_of_each_institute(records)